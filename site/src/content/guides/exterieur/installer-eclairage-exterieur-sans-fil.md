---
title: "Éclairage extérieur sans fil : guide complet pour illuminer ton jardin"
description: "Découvre comment installer un éclairage extérieur sans fil : solaire, LED à piles, connecté. Conseils, prix et erreurs à éviter."
type: guide
branch: exterieur
parent: exterieur
order: 1
...
```mermaid
graph TD
    A["Quel éclairage sans fil choisir ?"]
    A --> B{"Quel usage ?"}
    B -->|"Allée / chemin"| C["Bornes solaires à piquer"]
    B -->|"Terrasse / ambiance"| D["Guirlandes + lampes nomades"]
    B -->|"Mise en valeur végétaux"| E["Spots solaires orientables"]
    B -->|"Sécurité"| F["Projecteur solaire à détecteur"]

    C --> G["Budget : 25 - 50 euros"]
    D --> H["Budget : 80 - 150 euros"]
    E --> I["Budget : 20 - 130 euros/spot"]
    F --> J["Budget : 15 - 60 euros"]
...
```mermaid
graph LR
    A["Luminaire existant sur secteur"] --> B["Prise connectée IP44"]
    B --> C["App smartphone"]
    C --> D["Scénarios automatiques"]
    D --> E["Allumage au coucher du soleil"]
    D --> F["Extinction programmée"]
    D --> G["Détection de présence"]
