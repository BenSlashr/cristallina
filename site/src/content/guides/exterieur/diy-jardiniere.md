---
title: "DIY jardinière : 5 modèles faciles à fabriquer soi-même"
description: "Fabrique ta jardinière DIY en bois, palette, béton ou récup. 5 tutos simples avec matériaux, outils et étapes détaillées pour un résultat canon."
type: guide
branch: exterieur
parent: exterieur
order: 10
...
```mermaid
flowchart LR
    A[Préparer les moules] --> B[Couler le béton]
    B --> C[Attendre 24h]
    C --> D[Démouler]
    D --> E[Poncer & finir]
    E --> F[Imperméabiliser]
...
```mermaid
flowchart TD
    A[Coupe des planches] --> B[Assemblage du cadre]
    B --> C[Fixation des pieds]
    C --> D[Fond avec drainage]
    D --> E[Traitement bois]
    E --> F[Remplissage terre]
...
```mermaid
quadrantChart
    title Jardinières DIY - Coût vs Difficulté
    x-axis Facile --> Difficile
    y-axis Pas cher --> Cher
    quadrant-1 Ambitieux
    quadrant-2 Classique
    quadrant-3 Accessible
    quadrant-4 Technique
    Planches bois: [0.35, 0.35]
    Palette verticale: [0.15, 0.05]
    Béton moulé: [0.55, 0.25]
    Bac surélevé: [0.65, 0.7]
    Récup: [0.25, 0.15]
```
