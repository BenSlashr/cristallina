---
title: "Ombrager sa terrasse : 8 solutions comparées pour chaque budget"
description: "Parasol, pergola, voile d'ombrage ou plantes ? Compare les meilleures solutions pour ombrager ta terrasse selon ton budget et ton style."
type: guide
branch: exterieur
parent: exterieur
order: 8
...
```mermaid
graph TD
    A["Quel ombrage pour ta terrasse ?"]
    A --> B{"Budget ?"}
    B -->|"Moins de 200 euros"| C["Parasol déporté ou voile d'ombrage"]
    B -->|"200 - 1 000 euros"| D["Tonnelle renforcée ou pergola kit bois"]
    B -->|"Plus de 1 000 euros"| E["Pergola alu ou bioclimatique"]
    E --> F{"Tu veux du réglable ?"}
    F -->|Oui| G["Pergola bioclimatique à lames"]
    F -->|Non| H["Pergola alu avec toile rétractable"]
...
graph LR
    ...
    A --> C["Albizia"]
    A --> D["Tilleul"]
    B --> E["Port parasol - Rapide - Stérile"]
    C --> F["Léger - Élégant - Climat doux"]
    D --> G["Dense - Parfumé - Grand espace"]
...
```mermaid
graph TD
    A["Pas de perçage possible ?"]
    A --> B["Parasol déporté + lestage"]
    A --> C["Tonnelle autoportée"]
    A --> D["Voile sur mâts plantés"]
    A --> E["Canisses sur treillage mobile"]
    B --> F["Ombre ponctuelle - 150-800 euros"]
    C --> F2["Abri complet - 80-1200 euros"]
    D --> F3["Design - 50-200 euros"]
    E --> F4["Éco - 20-60 euros"]
    ...
    style E fill:#141D30,stroke:#8B5CF6,color:#F1F5F9
    style F fill:#141D30,stroke:#F59E0B,color:#F1F5F9
    style F2 fill:#141D30,stroke:#F59E0B,color:#F1F5F9
    style F3 fill:#141D30,stroke:#F59E0B,color:#F1F5F9
