---
title: "Decoration francaise romantique : idées et photos pour un intérieur poétique"
description: "Découvre les codes de la décoration française romantique : couleurs pastels, meubles patinés, tissus délicats et ambiance lumineuse pour un intérieur plein de charme."
type: guide
branch: decoration
parent: decoration
order: 5
...
```mermaid
flowchart TD
    A[Textiles romantiques]
    A --> B[Lit]
    A --> C[Fenetres]
    A --> D[Salon]

    B --> B1["Linge lavé stone (lin, coton)"]
    B --> B2["Dessus de lit matelassé"]
    B --> B3["Coussins brodés ou en velours"]

    C --> C1["Voilages en étamine ou mousseline"]
    C --> C2["Rideaux en lin naturel"]

    D --> D1["Jeté en mohair ou coton"]
    D --> D2["Coussins toile de Jouy"]
    D --> D3["Tapis à motifs floraux"]
    ...
    style B3 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
    ...
    style D3 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
```
...
flowchart LR
    ...
    A --> B[Floral]
    A --> C[Céramique]
    A --> D[Patine et vieilli]

    B --> B1["Pivoines, roses, eucalyptus"]
    B --> B2["Fleurs séchées dans brocs anciens"]

    C --> C1["Faïence blanche liséré"]
    C --> C2["Pots en grès naturel"]

    D --> D1["Cadres dorés et argentés"]
    D --> D2["Livres anciens en pile"]
    D --> D3["Coffrets en bois patiné"]
