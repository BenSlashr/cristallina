---
title: "Semer du gazon sans rouleau : toutes les techniques qui marchent vraiment"
description: "Tu veux une belle pelouse mais tu n'as pas de rouleau ? Pas de souci ! Découvre toutes les astuces pour semer ton gazon sans cet outil, avec des résultats au top."
type: guide
branch: decoration
parent: decoration
order: 1
...
```mermaid
flowchart TD
    A([Préparer le sol]) --> B([Épandre les graines])
    B --> C{Outil disponible ?}
    C -->|Planche de bois| D([Tassage par planche])
    C -->|Rien de spécial| E([Tassage à pied])
    C -->|Objet cylindrique| F([Tassage en roulant])
    D --> G([Couvrir d'une fine couche de terreau])
    E --> G
    F --> G
    G --> H([Arroser en pluie fine])
    H --> I([Maintenir humide 2-3 semaines])
    I --> J([Première tonte à 8-10 cm])
...
```mermaid
graph LR
    A([Type de gazon]) --> B([Familial / rustique])
    A --> C([Ornamental / fin])
    A --> D([Zone ombragée])
    B --> E(["Ray-grass + Fétuque élevée\nGermination : 7-14 jours"])
    C --> F(["Fétuque fine + Agrostis\nGermination : 14-21 jours"])
    D --> G(["Mélange ombre spécial\nGermination : 14-21 jours"])
