---
title: "Souder une gouttière en zinc : instructions pas à pas pour débutants"
description: "Apprenez à souder une gouttière en zinc toi-même : matériel, étapes détaillées, sécurité et astuces pour un résultat solide dès la première fois."
type: guide
branch: decoration
parent: decoration
order: 9
...
```mermaid
flowchart TD
    A["Rassembler le matériel"] --> B["Nettoyer la surface"]
    B --> C["Appliquer le flux décapant"]
    C --> D["Chauffer la zone avec fer ou chalumeau"]
    D --> E["Déposer l'étain"]
    E --> F["Laisser refroidir"]
    F --> G["Nettoyer le flux résiduel"]
    G --> H["Tester l'étanchéité"]
...
```mermaid
graph LR
    A["Surface non préparée
    - Oxydée / grasse
    - Sans flux
    Résultat : étain qui perle"] --> B["Surface bien préparée
    - Brossée + dégraissée
    - Flux appliqué
    Résultat : étain qui coule"]
...
```mermaid
flowchart TD
    A["Quel est ton projet ?"] --> B["Réparation fissure ou fuite"]
    A --> C["Assemblage de deux pièces"]
    A --> D["Raccord complexe (angle, jonction)"]

    B --> E["Fer à souder panne large
    + étain Sn99Cu1
    + flux neutre"]
    C --> F["Fer ou chalumeau
    + pince de maintien
    + chevauchement min 2 cm"]
    D --> G["Chalumeau + tiers expérimenté
    ou faire appel à un zingueur"]
