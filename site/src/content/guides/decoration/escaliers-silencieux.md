---
title: "Escaliers silencieux : comment éliminer les grincements pour de bon"
description: "Ton escalier grince à chaque pas ? Découvre toutes les techniques pour le rendre silencieux, du talc aux vis, sans gros travaux."
type: guide
branch: decoration
parent: decoration
order: 4
...
```mermaid
flowchart TD
    A[Escalier qui grince] --> B{Accès au-dessous ?}
    B -- Oui --> C[Intervention par dessous\nplus facile et propre]
    B -- Non --> D[Intervention par dessus\nvisible mais efficace]
    C --> E[Visser les tasseaux\nou les cales]
    C --> F[Colle à bois\ndans les jonctions]
    D --> G[Vis par la face\n+ bouchon bois]
    D --> H[Talc / graphite\ndans les fissures]
...
```mermaid
flowchart LR
    A[Marche qui bouge] --> B[Perçage en biais\nvis 3x40mm]
    B --> C{Finition}
    C -- Propre --> D[Tourillon bois\ncollé + poncé]
    C -- Rapide --> E[Pâte à bois\nteintée]
    D --> F[Escalier\nsilencieux]
    E --> F
    style A fill:#FFF0F5,stroke:#E879F9,color:#701A75
    style B fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
