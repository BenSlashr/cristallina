---
title: "Étagère murale IKEA : le modèle design et pratique qui se fixe en un clin d'oeil"
description: "Découvre l'étagère murale IKEA design et facile à installer. Pratique, abordable et stylée, elle s'adapte à toutes les pièces de la maison."
type: guide
branch: shopping
parent: shopping
order: 1
...
flowchart TD
    ...
    B -->|Oui| C["Cheville béton + vis inox"]:::secondary
    B -->|Non| D{"Placo / Cloison ?"}
    D -->|Oui| E["Cheville Molly ou à expansion"]:::secondary
    D -->|Non| F{"Bois / OSB ?"}
    F -->|Oui| G["Vis à bois directement"]:::secondary
    C --> H["Percer au diamètre indiqué"]:::principal
    E --> H
    G --> H
    H --> I["Fixer les consoles"]:::principal
    I --> J["Poser la tablette"]:::principal
    J --> K["Vérifier le niveau"]:::principal
    ...
    style K fill:#FFF0F5,stroke:#E879F9,color:#701A75
...
```mermaid
flowchart LR
    A["Salon"]:::principal --> A1["BERGSHULT large\nPour livres + déco"]:::secondary
    A --> A2["LACK plusieurs niveaux\nMur bibliothèque DIY"]:::secondary
    B["Chambre"]:::principal --> B1["FÄRGSÄTTARE\nAmbiance colorée"]:::secondary
    B --> B2["BURHULT étroite\nChevet suspendu"]:::secondary
    C["Cuisine"]:::principal --> C1["SKÅDIS panneau\nOrganisation maximale"]:::secondary
    C --> C2["VURM spécial\nPour épices / bocaux"]:::secondary
    D["Bureau"]:::principal --> D1["SKÅDIS\nAccessoires modulables"]:::secondary
    D --> D2["BERGSHULT avec\ngrande console"]:::secondary
    ...
    style A1 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
    style A2 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
    style B1 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
    style B2 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
    style C1 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
    style C2 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
    style D1 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
    style D2 fill:#F3F4F6,stroke:#8B5CF6,color:#581C87
```
