---
title: "Couleurs pour donner de la profondeur : murs et pièces transformés"
description: "Choisis les bonnes couleurs pour donner de la profondeur à un mur ou une pièce. Astuces illusions d'optique, teintes sombres et palettes efficaces."
type: guide
branch: couleurs
parent: couleurs
order: 10
...
```mermaid
graph TD
    A[Piece a transformer] --> B{Objectif ?}
    B --> C[Agrandir / ouvrir]
    B --> D[Creer de la profondeur]
    B --> E[Effet cocon / intimite]
    C --> F[Couleurs claires\nBlanc casse, beige, ivoire]
    D --> G[Mur du fond sombre\n+ autres murs clairs]
    E --> H[Teintes sombres\nsur tous les murs]
    F --> I[Reflet de lumiere\nespace percu plus grand]
    G --> J[Regard projete\nvers l'arriere]
    H --> K[Absorption lumiere\natmosphere feutree]
...
```mermaid
flowchart LR
    A[Bandes verticales] -->|Regard vers le haut| B[Piece plus haute]
    C[Bandes horizontales] -->|Regard vers les cotes| D[Piece plus large]
    E[Aplat total\nmurs + plafond] -->|Suppression des reperes| F[Espace continu\neffet design]
    G[Mur du fond sombre] -->|Regard projete en avant| H[Piece plus longue]
