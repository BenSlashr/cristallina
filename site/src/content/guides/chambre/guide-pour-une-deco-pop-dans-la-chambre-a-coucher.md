---
title: "Déco pop chambre : guide complet pour une ambiance colorée et vitaminée"
description: "Crée une chambre pop art pleine de couleurs et d'énergie : palettes, mobilier, affiches, textiles et astuces budget pour un résultat qui claque."
type: guide
branch: chambre
parent: chambre
order: 1
...
```mermaid
graph TD
    A["Ta palette pop"] --> B["1 couleur dominante"]
    A --> C["1 couleur secondaire"]
    A --> D["1 couleur d'accent"]
    B --> E["60% de la pièce - mur, literie"]
    C --> F["30% - mobilier, rideaux"]
    D --> G["10% - coussins, objets, art"]
...
```mermaid
graph LR
    A["Mobilier pop"] --> B["Vintage 60s-70s"]
    A --> C["Design actuel"]
    B --> D["Formes organiques"]
    B --> E["Pieds compas"]
    C --> F["Velours coloré"]
    C --> G["Acrylique transparent"]
...
```mermaid
graph TD
    A["Quel style pop pour toi ?"] --> B{"Aimes-tu les contrastes forts ?"}
    B -->|Oui| C{"Grande chambre ?"}
    B -->|Non| D["Pop pastel"]
    C -->|Oui| E["Pop maximaliste"]
    C -->|Non| F["Pop graphique N&B + 1 couleur"]
