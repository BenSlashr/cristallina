---
title: "Entretenir un bain nordique : guide complet pour le garder impeccable"
description: "Comment entretenir ton bain nordique au quotidien : traitement de l'eau, nettoyage du bois, hivernage. Tous les conseils pour en profiter longtemps."
type: guide
...
```mermaid
flowchart TD
    A([Bain nordique en service]) --> B{Fréquence d'utilisation}
    B --> |Quotidienne| C[Vérifier pH + chlore chaque jour]
    B --> |2-3 fois/semaine| D[Contrôle 2x/semaine]
    B --> |Occasionnelle| E[Contrôle avant chaque bain]
    C --> F[Choc chlore hebdomadaire]
    D --> F
    E --> G[Renouveler l'eau si >3 mois]
    F --> H([Eau équilibrée et saine])
    G --> H
...
```mermaid
graph LR
    A([Eau sale ou trouble]) --> B[Vérifier le filtre]
    B --> C{État du filtre}
    C --> |Encrassé| D[Rincer ou remplacer]
    C --> |Propre| E[Choc chlore + flocculant]
    D --> F[Laisser la pompe tourner 4h]
    E --> F
    F --> G{Eau claire ?}
    G --> |Oui| H([Bain prêt])
    G --> |Non| I[Vidanger et remplir]
...
```mermaid
gantt
    title Planning d'entretien mensuel bain nordique
    dateFormat  DD
    section Hebdomadaire
    Test pH + chlore          :active, a1, 01, 2d
    Test pH + chlore          :active, a2, 08, 2d
    Test pH + chlore          :active, a3, 15, 2d
    Test pH + chlore          :active, a4, 22, 2d
    Choc chlore               :crit, b1, 07, 1d
    Choc chlore               :crit, b2, 14, 1d
    Choc chlore               :crit, b3, 21, 1d
    Choc chlore               :crit, b4, 28, 1d
    section Mensuel
    Rinçage filtre             :c1, 05, 2d
    Nettoyage couvercle        :c2, 19, 2d
    section Trimestriel
    Vidange complète           :d1, 01, 3d
```
